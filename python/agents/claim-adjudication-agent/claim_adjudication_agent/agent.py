...
from .tools.tools import getClaimsDetails, after_tool_callback
...
claims_processing_core_agent = ParallelAgent(
    ...
    description = "Facilitates the concurrent execution of core claim evaluation modules, including admissibility verification and financial adjudication, to ensure efficient processing.",
...
claims_processing_flow_agent = SequentialAgent(
    ...
    description = "Manages the sequential progression of claim processing, transitioning from core technical adjudication to final report synthesis and summary generation.",
...
cashless_health_claim_advisor_workflow = LlmAgent(
    ...
    model=GEMINI_FLASH,
    description="The master orchestration agent responsible for the end-to-end management of the cashless health insurance claim lifecycle, streamlining document retrieval, verification, and adjudication.",
    instruction=CASHLESS_HEALTH_CLAIM_ADVISOR_WORKFLOW_PROMPT,
    sub_agents = [claims_processing_flow_agent],
    tools=[FunctionTool(getClaimsDetails)],
